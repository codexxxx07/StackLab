...
export default function Footer() {
  return (
    <footer className="mt-auto border-t-[3px] border-ink bg-cream">
      <div className="mx-auto max-w-7xl px-4 py-8 lg:px-8">
        <div className="grid gap-8 md:grid-cols-3">
          <div>
            <div className="mb-3 flex items-center gap-2">
              <div className="flex h-8 w-8 items-center justify-center border-[3px] border-ink bg-grape font-display text-sm text-white shadow-pop-xs">
                S
              </div>
              ...
              {CONVERSIONS.map((c) => (
                <li key={c.id}>
                  ...
                  >
                    {c.title}
                    {c.status === 'soon' && (
                      <span className="ml-2 inline-block border border-ink bg-lemon-soft px-1.5 py-0.5 text-[10px] font-bold uppercase">
                        Soon
                      </span>
                    )}
                  </Link>
        ...
        <div className="mt-8 border-t-[3px] border-ink pt-4 text-center">
